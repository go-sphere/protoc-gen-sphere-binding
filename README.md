...
`protoc-gen-sphere-binding` is a protoc plugin that adds Sphere binding struct tags to the Go structs generated by `protoc-gen-go` from `.proto` files. It reads the `sphere.binding.*` annotations and rewrites the generated `.pb.go` files in place, so it always runs after `protoc-gen-go`. Inspired by [protoc-gen-gotag](https://github.com/srikrsna/protoc-gen-gotag).
...
`default_location` sets the binding location for every field of a message, and `default_oneof_location` does the same for the members of a oneof. Both accept the same `BindingLocation` values as `location`, and an explicit `(sphere.binding.location)` on a field still wins:

```protobuf
message SearchRequest {
  option (sphere.binding.default_location) = BINDING_LOCATION_QUERY;

  string query = 1;   // Gets query:"query"
  int32 limit = 2;    // Gets query:"limit"
}
```

Nested messages do not inherit the enclosing message's default location: a nested type is tagged only when it declares its own annotations, so a JSON body field of a nested type keeps its `json` tag.
...
message EventRequest {
  oneof payload {
    ...
    string text = 1;    // Gets json:"-" query:"text"
    int64 number = 2;   // Gets json:"-" query:"number"
  }
}
```

> Proto3 `optional` fields are backed by a synthetic oneof. They are treated as ordinary fields and use their `location` / `default_location`, not the oneof options.
